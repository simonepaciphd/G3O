...
Status: SIGNED OFF by PI 2026-07-06 (decisions recorded below). Drafted
2026-07-05 on branch `feature/eligibility-filter-design`, baseline main @
`85fc694`. Implementation delegated to the Data Validation Team RAs — see
`RAs/Data Validation Team/instruction-briefs/2026-07-06 Eligibility Filter and
First Live Funnel Runs.md` (Drive).
...
the URL string alone. Language-neutral by construction.
...
- **Shadow recall metric** (the enable-decision input): after Stage 3, compute
  per-language `would_drop ∩ llm_keep / llm_keep`. Caveat from fact 1b: this
  measures *disagreement* with a URL-only judge; the PI reviews a sample of
