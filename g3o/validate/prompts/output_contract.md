...
This contract sits downstream of the G3O Output Contract v2.3 (`g3o/extract/prompts/output_contract.md`). Vocabulary references (source-credibility tiers, uncertainty-flag vocabulary, `genai_evidence` semantics, controlled enum values) inherit from v2.3; this document only specifies what changes for the consolidated shape.
...
| 22 | uncertainty_flags | string | Semicolon-joined union, no spaces; or `none` | Order alphabetically. See vocabulary in v2.3 §4.10. |
...
| 7 | source_type | enum | `official_gov` / `procurement_tender` / `news_major` / `news_trade` / `vendor` / `academic` / `policy_org` / `social_media` / `archive` / `other` | See v2.3 §4.6 |
| 8 | source_language | string | ISO 639-1 | Language of the source page |
| 9 | source_credibility | enum | `high` / `medium` / `low` | Per v2.3 §4.8 hierarchy |
