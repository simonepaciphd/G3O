...
1a/1b ``language`` tags and the Stage-3 ``3_triage.json`` decisions) and reports:
  ...
  * **shadow recall** per language — ``would_drop ∩ llm_keep / llm_keep`` — the
    enable-decision input from the memo. Caveat (memo fact 1b): this measures
    *disagreement* with a URL-only LLM judge, not filter error. It is only
    meaningful in ``shadow`` mode, where Stage 3 still sees the would-drop URLs;
    under ``enforce`` those URLs never reach Stage 3 so the intersection is 0 by
    construction.
"""
...
def compute_filter_block(
    ...
    for lang, b in sorted(per_language.items()):
        per_lang_out[lang] = {
            ...
            "would_drop_and_llm_keep": b["_would_drop_and_llm_keep"],
            # would_drop ∩ llm_keep / llm_keep — the disagreement rate.
            "shadow_recall": _pct(b["_would_drop_and_llm_keep"], b["_llm_keep"]),
        }
    ...
    return {
        ...
        "flag": flag,
        "note": (
            "Shadow recall is disagreement with a URL-only LLM judge, not filter "
            "error (memo fact 1b); review a sample before treating it as error. "
            "In shadow mode nothing is dropped."
        ),
    }


__all__ = ["compute_filter_block", "FILTER_WOULD_DROP_WARN_PCT"]
