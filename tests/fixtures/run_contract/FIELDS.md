...
9. **`stage` ∈ the 8-name roster** (`config.py:26`), in roster order: `discovery_general`, `classify_official_site`, `discovery_site_restricted`, `filter_eligibility`, `classify_triage`, `scrape`, `extract`, `validate` — **or one of the three named sub-steps** the four-leg pipeline (2026-09-03) runs inside roster stages: `discovery_general_fallback` and `classify_official_site_fallback` (inside `classify_official_site`), `discovery_evidence_open` (inside `discovery_site_restricted`). Sub-steps carry `stage_started`/`stage_completed` (and the fallback classify carries `chunk_*`) but are not roster stages: `stages_planned` still lists eight. The loader accepts exactly these names (`run_contract.SUBSTEPS`, g3o-api #38) and refuses anything else. Only the LLM stages (`classify_official_site`, `classify_official_site_fallback`, `classify_triage`, `extract`, `validate`) emit `chunk_*`.
